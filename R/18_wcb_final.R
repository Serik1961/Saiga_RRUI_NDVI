...
# 18_wcb_final.R  (rewritten in v1.3.1)
#
# Final model M3 (+ livestock)
...
wcb <- wcb_exact(panel, c("RRUI", "precip_mm", "temp_c", "Livestock_units"))

cat("\n===== Final model M3 (+ livestock): exact WCB =====\n")
print(wcb, digits = 5)
...
write_csv(wcb, "results/tables/18_final_WCB.csv")
write.xlsx(wcb, "results/tables/18_final_WCB.xlsx", overwrite = TRUE)

# Exact WCB for every coefficient of the model (v1.3.1)
vars_all <- c("RRUI", "precip_mm", "temp_c", "Livestock_units")
...
write_csv(wcb_all, "results/tables/18_final_WCB_all_coefficients.csv")

cat("\nSaved: results/tables/18_final_WCB.csv\n")
