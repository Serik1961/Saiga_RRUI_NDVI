...
# Исходные GPS-маршруты имеют координаты WGS 84 / UTM zone 38N.
# v1.3.1: CRS проверяется (см. R/helpers_gps_crs.R); при необходимости
# выполняется st_transform(). Идентификатор EPSG:32638 присваивается
# только если метаданные слоя подтверждают UTM zone 38N.

source("R/helpers_gps_crs.R")

routes_utm <- standardise_route_crs(
  routes_indyear,
  32638
)
